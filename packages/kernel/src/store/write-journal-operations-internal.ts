...
function machineArtifactWriteDescriptor(op: WriteOp): MachineArtifactWritePayload {
  const payload = op.payload;
...
function resolveMachineArtifactWrite(rootInput: HarnessLayoutInput, op: WriteOp): { readonly targetPath: string; readonly body: string; readonly bodyRef?: ContentAddressedBlobRef } {
