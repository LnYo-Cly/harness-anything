...
import { commandParserIds, commandRunnerIds } from "./command-dispatch-ids.ts";
...
const commandUsages = [
  ...
  { kind: "new-task", usage: "task create --title <title> [--parent <task-id>] [--kind feat|fix|refactor|docs|test|chore] [--risk-tier low|medium|high] [--urgency low|medium|high] [--from-file <path>|--json-input <json>] [--vertical software/coding --preset <id> --module <key>] [--register-module <key> --module-title <title> --module-scope <path>] [--long-running] [--dry-run] [--locale zh-CN|en-US] [--from-legacy <legacy-id>] [--json]", aliases: [deprecatedAlias("new-task --title <title>", "task create")] },
  { kind: "status-set", usage: "task transition <id> <planned|active|blocked|in_review|done|cancelled> [--force --reason <reason>]", aliases: [deprecatedAlias("task status set <id> <status>", "task transition")] },
  ...
  { kind: "task-tree", usage: "task tree <id> [--json]" },
  { kind: "task-relate", usage: "task relate <source-task-id> depends-on <target-task-id> --rationale <text> [--dry-run] [--json]" },
  ...
  { kind: "decision-accept", usage: "decision accept <decision-id> [--arbiter kind:id] [--decided-at <iso>] [--judgment-only <rationale>] [--dry-run] [--json]" },
  ...
  { kind: "decision-amend", usage: "decision amend <decision-id> [--title <title>] [--load-bearing <claim-id>|--non-load-bearing <claim-id>] [--set <field>:<value>] [--append <field>:<json>] [--body <text>] [--dry-run] [--json]" },
  { kind: "decision-relate", usage: "decision relate <decision-id> --anchor <CH1|RJ1|C1> --type <supports|supersedes|refines|narrows|derives|blocks|relates|implements|produces|evidences|evidenced-by|invalidated-by|supersedes-fact> --target <entity-ref> --rationale <text> [--body <text>] [--dry-run] [--json]; derives->task seeds missing task risk-tier/urgency once, not live sync" },
  { kind: "decision-reckon", usage: "decision reckon <decision-id> --task <task-id> [--dry-run] [--json]" },
  { kind: "decision-relation-retire", usage: "decision relation retire <decision-id> --relation <relation-id> [--body <text>] [--dry-run] [--json]" },
  { kind: "decision-relation-replace", usage: "decision relation replace <decision-id> --relation <relation-id> --anchor <CH1|RJ1|C1> --type <supports|supersedes|refines|narrows|derives|blocks|relates|implements|produces|evidences|evidenced-by|invalidated-by|supersedes-fact> --target <entity-ref> --rationale <text> [--body <text>] [--dry-run] [--json]; derives->task seeds missing task risk-tier/urgency once, not live sync" },
  { kind: "decision-retire", usage: "decision retire <decision-id> [--arbiter kind:id] [--decided-at <iso>] [--dry-run] [--json]" },
  { kind: "fact-list", usage: "fact list --task <task-id> [--json]" },
  { kind: "fact-show", usage: "fact show --task <task-id> --id <fact-id> [--json]" },
  ...
  { kind: "fact-invalidate", usage: "fact invalidate --task <task-id> --id <fact-id> --by <fact-id> --rationale <text> [--dry-run] [--json]" },
  ...
  { kind: "task-list", usage: "task list [--state <state>] [--module <key>] [--queue <queue>] [--preset <id>] [--kind feat|fix|refactor|docs|test|chore] [--risk-tier low|medium|high] [--urgency low|medium|high] [--review <state>] [--lesson [present|missing]] [--missing-materials] [--include-archived] [--search <text>] [--json]" },
...
const commandSummaries = {
  ...
  "task-tree": "Show a task subtree derived from the parent field projection.",
  "task-relate": "Append a task->task depends-on relation without scheduling or status side effects.",
  ...
  "decision-accept": "Accept a proposed decision through the decision write service after the non-empty evidence floor or an explicit judgment-only rationale.",
  ...
  "decision-reckon": "Evaluate load-bearing decision claim coverage and record the verdict as a task-local fact.",
  "decision-relation-retire": "Retire a hosted decision relation by rewriting the source decision frontmatter.",
  "decision-relation-replace": "Replace a hosted decision relation by retiring the old edge and appending a new edge.",
  "decision-retire": "Retire a decision through the decision write service.",
  "fact-list": "List task-local fact anchors from the facts document.",
  "fact-show": "Show one task-local fact anchor by id.",
  "record-fact": "Record a stable task-local fact anchor through the fact write service.",
  "fact-invalidate": "Invalidate a task-local fact by recording a superseding fact relation through the fact write service.",
...
const commandExamples = {
  ...
  "new-task": [`${cliCommandName} task create --title "Normalize CLI help" --parent task_01ABC --vertical software/coding --preset standard-task`],
  ...
  "task-tree": [`${cliCommandName} task tree task_01ABC --json`],
  "task-relate": [`${cliCommandName} task relate task_01ABC depends-on task_01DEF --rationale "ABC waits for DEF"`],
  ...
  "decision-propose": [`${cliCommandName} decision propose --title "Adopt CLI decision loop" --question "Should M3 expose decision CLI?" --chosen "Expose it" --rejected "Keep write API only" --why-not "No human fallback path" --evidence-relation C1:supersedes-fact:fact/task_01ABC/F-1234ABCD:"Evidence covers C1."`],
  ...
  "decision-reckon": [`${cliCommandName} decision reckon dec_01ABC --task task_01ABC`],
  "decision-relation-retire": [`${cliCommandName} decision relation retire dec_01ABC --relation rel_0123456789abcdef`],
  "decision-relation-replace": [`${cliCommandName} decision relation replace dec_01ABC --relation rel_0123456789abcdef --anchor CH1 --type relates --target decision/dec_00XYZ --rationale "Replacement edge."`],
  "decision-retire": [`${cliCommandName} decision retire dec_01ABC --arbiter human:ZeyuLi`],
  "fact-list": [`${cliCommandName} fact list --task task_01ABC --json`],
  "fact-show": [`${cliCommandName} fact show --task task_01ABC --id F-DEADBEEF --json`],
  ...
  "fact-invalidate": [`${cliCommandName} fact invalidate --task task_01ABC --id F-DEADBEEF --by F-FEEDFACE --rationale "New fact supersedes old fact"`],
