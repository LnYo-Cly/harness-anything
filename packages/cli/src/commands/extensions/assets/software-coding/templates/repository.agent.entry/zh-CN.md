...
- Task progress: `ha task status set <id> active`, then `ha task progress append <id> --text "<记录>" --evidence type:PATH:summary`. Progress is a timeline, not a fact ledger.
- Fact gate: every task that reaches review/complete must have at least one real fact. Record load-bearing observations with `ha record fact --task <id> --statement "<可复核观察>" --source "<来源>" --confidence high`.
