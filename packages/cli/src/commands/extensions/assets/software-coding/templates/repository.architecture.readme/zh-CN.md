...
Authored model 只通过显式、经过 review 的修改更新。Snapshot 只在所属 task 运行 `architecture-snapshot` 时刷新，比较只在运行 `architecture-check` 时发生。没有后台进程自动改写任何一方，import 扫描不能自行晋升为架构意图，这两个命令也都不是通用 completion 或 CI gate。
...
每个 coding task 开始时、广泛搜索源码前，先检查 manifest。启用时读取本入口，并且只加载回答最小必要问题所需的 view/flow：哪个 stable node 负责该行为、直接 incomers/outgoers 是什么、哪些多跳路径受影响，以及为什么所选实现层级是 canonical owner。跨模块、陌生区域、写路径、runtime boundary、依赖方向或 owner 不明确的任务必须深入查询；docs-only 或明确局部低风险的工作可以写明理由后标 N/A。task 的 `code-impact-analysis.md` 只记录稳定引用和当前 snapshot digest。

如果源码搜索出现多个可能的 owner/层级、incomers/outgoers 不确定或文档互相冲突，继续搜索前先回到地图，并按 stable node → view/flow → source scope → code 的顺序恢复定位。
