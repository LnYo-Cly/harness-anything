...
test("CLI milestone-closeout exposes v3 agent guidance without a machine-verdict script", () => {
  const guidance = readFileSync(path.join(presetRoot, "PRESET.md"), "utf8");
  assert.match(guidance, /agent performs the review/u);
  assert.match(guidance, /does not produce a machine verdict/u);
  ...
  withTempRoot((rootDir) => {
    ...
    assert.match(String(summary.description), /Verify milestone criteria/u);
    assert.match(String(summary.whenToUse), /milestone wrap-up/u);
    ...
    const created = runJson(rootDir, [
      "task", "create",
      "--title", "Guided milestone closeout",
...
function withTempRoot<T>(fn: (rootDir: string) => T): T {
  const rootDir = mkdtempSync(path.join(tmpdir(), "ha-milestone-closeout-guidance-"));
  try {
