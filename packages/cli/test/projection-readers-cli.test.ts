...
test("CLI exposes the projection-backed session execution task review and audit read surface", () => {
  withTempRoot((rootDir) => {
    ...
    const legacySessionTrace = runJson(rootDir, ["session", "trace", sessionId]);
    const sessionTrace = runJson(rootDir, ["session", "show", sessionId, "--view", "trace"]);
    assert.deepEqual(sessionTrace, legacySessionTrace);
    assert.equal(sessionTrace.report.trace.executions[0].executionId, executionId);
    ...
    const legacyTaskTrace = runJson(rootDir, ["task", "trace", taskId]);
    const taskTrace = runJson(rootDir, ["task", "show", taskId, "--view", "trace"]);
    assert.deepEqual(taskTrace, legacyTaskTrace);
    assert.equal(taskTrace.report.trace.executions[0].reviews[0].reviewId, reviewId);
