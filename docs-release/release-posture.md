...
| ---------------------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Subtask expansion preset     | Guidance           | The preset guides an agent to inspect the parent, create child tasks, and add real dependency relations with the normal `ha task` commands. It has no headless expansion entrypoint. |
...
| Code-doc reconciliation gate | Mechanism-complete | When the resolved preset/profile declares `code-doc-reconciliation` (as bundled coding profiles do), `ha task complete` hard-fails until `ha task code-doc reconcile <id> --commit <full-sha> --path <repo-relative-path>` generates and validates the reserved `harness/tasks/<id>/code-doc-anchors.json`; do not hand-author it. Task creation does not generate it. The gate is contract-derived, not universal (ADR-0027 D7). Evidence: canon 1.4. |
...
| Create-milestone preset      | Guidance           | The preset guides an agent to create the long-running root task and maintain the milestone map under the configured repository-relative milestones root. It has no headless scaffold/check entrypoint. |
