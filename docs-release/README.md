...
`ha task complete` still requires substantive closeout content, code-doc
reconciliation, and the applicable review contract (including an approved
Review for an Execution-bearing task). It does not require any minimum number
of Facts: under `dec_mrg3z1we/CH4`, Facts are explicit `0..N` promotions and are
never generated by submit, review, or complete.
