...
| CI gate axis | the CLI-provided CI gate must be `passed` | `ci_not_passed` |
...
The code-doc reconciliation file is not generated by `ha task create`; it must be
authored into the task package as `harness/tasks/<id>/code-doc-anchors.json`.
